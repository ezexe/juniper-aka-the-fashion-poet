...
namespace Juniper.WinUI.App.Data
{
    /// <summary>
    ...
    {
        public ControlInfoDataItem(string uniqueId, string title, Symbol img):this(uniqueId, title, "", "" + ((int)img), "", "", "", false, false, false)
        {
            ImageSymbol = img;
        }
        ...
        public Symbol ImageSymbol { get; private set; }
    ...
    {
        public ControlInfoDataGroup(string uniqueId, string title, Symbol img): this(uniqueId, title, "sb", "" + ((int)img), "desc")
        {
            ImageSymbol = img;
        }
        ...
        private IconElement GetIcon(string imagePath)
        {
            return imagePath.ToLowerInvariant().EndsWith(".png") ?
                        ...
                        (IconElement)new SymbolIcon() { Symbol = (Symbol)System.Enum.Parse(typeof(Symbol), imagePath) };
        }
        public override string ToString()
        {
            return this.Title;
    ...
    public sealed class ControlInfoDataSource
    {
        ...
        private async Task GetControlInfoDataAsync()
        {
            ...
            //Uri dataUri = new Uri(Path.Combine(Package.Current.InstalledLocation.Path, "Data", "ControlInfoData.json"));
